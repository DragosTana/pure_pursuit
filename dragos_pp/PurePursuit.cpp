    ...
    for (int i = this->last_closest_index_; i < path.size() - 1; i++) {
        ...
        if (discriminant >= 0) {
            ...
            if (t1 >= 0 && t1 <= 1) {
                ...
                Point intersection_point = segment_start + d * t1; // LOL QUESTA ROBA NON E SIMMETRICA RISPETTO ALL'OPERATOR OVERLOADING

                // Check if the intersection point is ahead of the last lookahead point
                if (i + t1 > fractional_lookahead_index_) {
                    this->fractional_lookahead_index_ = i + t1;
            ...
            if (t2 >= 0 && t2 <= 1) {
                ...
                if (i + t2 > fractional_lookahead_index_) {
                    this->fractional_lookahead_index_ = i + t2;
                    return intersection_point;
                }
            }
        }
...
double PurePursuit::calculate(const Point& position, const Point& goal_point) {
    return 0.0;
}